 ...
 * recreate the lightbox and leave stray `.lg-container` nodes behind. The
 * only prop here is a ref, whose identity never changes, so this subtree
 * renders once and Isotope handles all filtering imperatively after that.
 */
        ...
        {galleryPage.items.map((item) => (
          ...
          >
            ...
            >
              ...
              >
                <Expand
                  className="w-6 h-6 text-white opacity-0 scale-75 transition-all duration-500 group-hover:opacity-100 group-hover:scale-100"
                  strokeWidth={1.5}
                />
              </span>
            </a>
    ...
    <section className="relative pb-24 lg:pb-32">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex flex-wrap justify-center gap-3 mb-12 animate-fade-in-up delay-300">
          {galleryPage.categories.map((cat) => (
            <button
              key={cat.key}
              type="button"
        ...
        <GalleryLightboxGrid gridRef={gridRef} />
      </div>
    </section>
  );
}
