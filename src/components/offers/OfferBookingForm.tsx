    ...
    <>
      <form className="space-y-5" onSubmit={handleSubmit(onValid)} noValidate>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <div id="booking-price" className="flex items-center gap-2 h-13">
              <i
                className="fa-solid fa-tag text-base text-luxury-muted shrink-0"
                aria-hidden="true"
              />
        ...
        <div className="block my-10">
          <span className="luxury-label text-[11px] text-luxury-charcoal block w-full">
            Total Amount
          </span>
        ...
        <div>
          <span className="luxury-label text-[11px] text-luxury-charcoal block mb-3">
              ...
              return (
                <button
                  ...
                  className={`relative flex items-start gap-3 rounded-2xl border px-5 py-4 text-left transition-colors cursor-pointer ${
                    active
                      ? "border-(--color-primary-green) bg-(--color-primary-green)/5"
                      : "border-hairline hover:border-soft"
                  }`}
                >
                  <span
                    className={`flex w-10 h-10 shrink-0 items-center justify-center rounded-full transition-colors ${
                      active
                        ...
                        : "bg-luxury-cream text-luxury-muted"
                    }`}
                  >
                    <i
                      className={`fa-solid fa-${icon} text-base`}
                      aria-hidden="true"
                    />
                  </span>
                  <span className="flex-1 min-w-0 pr-2">
                    <span className="block text-sm font-medium text-luxury-charcoal">
                      {label}
                    </span>
                    <span className="block text-xs text-luxury-muted mt-0.5 leading-snug">
                      {description}
                    </span>
                  </span>
                  <span
                    className={`absolute top-4 right-4 w-5 h-5 shrink-0 rounded-full border flex items-center justify-center transition-colors ${
                      active
                        ? "border-(--color-primary-green) bg-(--color-primary-green)"
                        : "border-hairline"
                    }`}
                  >
                    {active && (
        ...
        {submitted && (
          <p className="text-sm text-luxury-muted text-center">
            Thank you — your booking request has been received. We&apos;ll be in
