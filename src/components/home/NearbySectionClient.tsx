...
export default function NearbySectionClient({ items }: { items: NearbyItem[] }) {
  // `null` = the hotel itself, which is what the map shows on first paint.
  const [activeTitle, setActiveTitle] = useState<string | null>(null);
  const active = items.find((item) => item.title === activeTitle);
  const query = active ? active.query : HOTEL_QUERY;
  // The CMS ships a ready-made embed URL per landmark that shows the route
  // and distance from the hotel — always use it for a selected landmark.
  // embedSrc only ever renders the hotel's own pin (no CMS record exists for
  // that), so it's never used to fake a per-landmark distance view.
  const iframeSrc = active?.mapUrl || embedSrc(HOTEL_QUERY, 15);
  const openInMapsHref = active?.mapUrl || mapsLink(query);
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="grid lg:grid-cols-[22rem_1fr] xl:grid-cols-[24rem_1fr] gap-6 lg:gap-8 animate-fade-in-up delay-100">
                ...
                <li key={item.title}>
                  ...
                  >
                    ...
                    <span className="mt-1.5 flex items-center gap-2 text-sm text-luxury-muted">
                      ...
                      <i
                        aria-hidden="true"
                        className={`fa-solid fa-arrow-right text-base transition-transform duration-300 ${
          ...
          <div className="relative h-88 lg:h-128 luxury-media border border-hairline">
            ...
            >
              <i className="fa-solid fa-location-dot text-sm text-gold" aria-hidden="true" />
              Open In Maps
