...
export default function DestinationSectionClient({ items }: { items: NearbyItem[] }) {
  ...
  // The CMS ships a ready-made embed URL per landmark — prefer it over
  // building one from a search query, which is only a fallback for the
  // hotel-itself pin (never CMS-backed).
  const iframeSrc = active?.mapUrl || embedSrc(query, active ? 14 : 15);
